...
export const ChatProvider = ({ children }) => {
    ...
    const sendMessage = async (receiverId, text, jobId = null) => {
    ...
    const getChats = () => {
        ...
        messages.forEach(msg => {
            ...
            if (!chatsMap.has(chatId)) {
                ...
                chatsMap.set(chatId, {
                    ...
                    jobId: msg.job_id,
                    messages: [],
                    company: otherName || (job ? job.company : (isEmployer ? "Соискатель" : "Работодатель")),
        ...
        if (activeChatId && !chatsMap.has(activeChatId)) {
            ...
            chatsMap.set(activeChatId, {
                ...
                jobId: null,
                messages: [],
                company: otherName || "Работодатель",
                isEmployer: false,
                otherPartyAvatar: otherProfile?.avatar_url || null
            });
        }
    ...
    const startChat = (ownerId) => {
        ...
        if (!profilesMap.has(ownerId)) {
            supabase.from('profiles').select('*').eq('id', ownerId).single().then(({ data }) => {
                if (data) setProfilesMap(prev => new Map(prev).set(data.id, data));
            });
        }
    };
